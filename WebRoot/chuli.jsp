  ...
  <head>
   ...
   <script type="text/javaScript">
  	function checksid()
  	{
  		...
  		if(temp==1)
		{
  			shanchu_submit.submit();	
		}else{
			alert("该药品还没有过期或者药品已经不存在喽~");
		}
  	}
  	function checkgid()
  	{
...
<p>过期药品处理</p>
<center>
<form action="chuli" method="post" name="shanchu_submit">
	请输入需要删除过期的药品编号：<input type="text" name="sid" id="sid"/><input type="button" value="删除" onclick="checksid();"/>
	
</form>
<form action="addguoqi" method="post" name="addguoqi_submit">
	请输入需要添加的过期药品编号:<input type="text" name="gid" id="gid"/><input type="button" value="添加" onclick="checkgid();"/>
</form>
<%
...
%>
<font size="5px" color="red"><%=(String)request.getAttribute("fanhui")%></font>
<%} %>
</center>
<table border="1">
	<tr>
